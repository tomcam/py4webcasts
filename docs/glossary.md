...
By default py4web uses a package called [PyDal](#pydal) for its DAL, though you aren't required to use PyDal.

## Field()

The `Field()` function is passed to the [PyDal](#pydal) `define_table()` function and represents both a field (aka column)
